...
import { TrajectoryPoint, GNSSData, DeadReckoningState } from '../types';
...
export function useDemoMode() {
  ...
  const start = useCallback(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (gnssEnabledRef.current) {
      ...
      } else if (!gnssEnabledRef.current && state.mode !== 'GNSS_RECOVERING' && state.mode !== 'FUSED') {
        // DEAD RECKONING mode
        ...
        dispatch({ type: 'UPDATE_GNSS', data: gnssData as GNSSData });
        const drState = deadReckoningEngine.getState();
        const fusionState = sensorFusionEngine.fuse(gnssData as GNSSData, drState, state.fusion);
        ...
        if (sensorFusionEngine.getProgress() > 0.8 && state.mode !== 'FUSED') {
  ...
  const resume = useCallback(() => {
    if (!isRunningRef.current) {
      isRunningRef.current = true;
      // Restart interval from current index
      start();
    }
  }, [start]);
